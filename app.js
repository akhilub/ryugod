...
hash = () => {
  return crypto
    .createHmac("sha256", "mysharedsource01")
    ...
    if (fs.existsSync(path, { encoding: "utf-8" })) {
      ...
      for (file of files) {
        ...
        if (fs.existsSync(fpath, { encoding: "utf-8" })) {
          ...
          if (file.slice(-3) === ".png") continue

          file_list.push(
            `<url><loc>https://${host}${fpath
              .replace(base_dir, "")
              .replace(/\/contents\//, "/pages/")}</loc>` +
      ...
      fs.readdir(path, { encoding: "utf-8" }, (err, files) => {
        ...
        files.map((file) => {
          ...
          if (isDir || file.endsWith(".md") || file.endsWith(".template")) {
            subfiles = isDir
              ? fs.readdirSync(path + "/" + file, { encoding: "utf-8" })
              : []
            subfiles.sort()
            subfiles = subfiles.filter(
              (f) => f.endsWith(".md") || f.endsWith(".template")
            )
...
websocketServer.on("connection", (ws, req) => {
  ...
  const dockerImage =
    query.dockerImage &&
    query.dockerImage != "undefined" &&
    query.dockerImage != "null"
      ? query.dockerImage
      : "ubuntu:ryugod"
  console.log(
    new Date().toString(),
